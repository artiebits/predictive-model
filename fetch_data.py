...
competitions = dict(
    ...
    epl=dict(name="Premier League", page="englandm.php"),
)


def fetch_data(competition):
    # Connect to football-data.co.uk
    res = requests.get("http://www.football-data.co.uk/" + competition["page"])
    ...
    for url in data_urls:
        ...
        print(f"Getting data for {competition['name']} season {season}")

        # Save the data from the url into csv files
...
def fetch_laliga_data():
    return fetch_data(competitions["laliga"])


def fetch_epl_data():
    return fetch_data(competitions["epl"])
